...
def transaction_scope():
    ...
    Session = sessionmaker(bind=db.engine)  # Create a session class
...
def get_action_keys(action, session):
    return session.query(ZoneKey).filter(ZoneKey.registrar_action == action).all()
...
def unlink_action_keys(action_keys, session):
...
def handle_pending():
    while True:
        with transaction_scope() as session:
            action = session.query(RegistrarAction).filter_by(status='pending').first()
            if action is None:
                break
            try:
                ...
                if got_notification(et.fromstring(reply)):
                    ...
                    unlink_action_keys(get_action_keys(action, session), session)
                session.commit()
            except Exception:
...
def poll_queue(account):
    with transaction_scope() as session:
        running = session.query(RegistrarAction).filter_by(status='running').join(Zone).join(
            RegistrarAccount) \
    ...
    while True:
        ...
        if ctid is not None:
            with transaction_scope() as session:
                action = session.query(RegistrarAction).filter_by(id=int(ctid.text)).first()
                if action is None:
                    logging.warn('Ignoring registrar-account %s reply for action with id %s' %
                                 (account['name'], ctid.text))
                else:
                    action.completed = datetime.datetime.utcnow()
                    action_keys = list(get_action_keys(action, session))
                    if is_success_message(pi):
                        ...
                        logging.info('Domain update for %s failed with registrar-account %s' %
                                     (action.zone.display_name, action.zone.registrar_account.name))
                    unlink_action_keys(action_keys, session)
                    session.commit()
...
def discard_old_operations():
    ...
    '''
    with transaction_scope() as session:
        actions = session.query(RegistrarAction).filter_by(status='running') \
...
def transaction_scope():
    """Provide a transactional scope around a series of operations."""
    session = db.session()  # Get a session instance
    try:
        yield session  # Pass the session to the context
        session.commit()
    except Exception:
        session.rollback()
        raise
...
def poll_queue(account):
    with transaction_scope() as session:
        running = session.query(RegistrarAction).filter_by(status='running').join(Zone).join(RegistrarAccount)\
            .filter(RegistrarAccount.id == account['id']).count()
        if running == 0:
            return
    while True:
...
def run():
    ...
    while True:
        ...
        accounts = []
        with transaction_scope() as session:
            for account in session.query(RegistrarAccount).all():
