...
class Vlan(db.Model):
    ...
    vid: Mapped[Integer] = mapped_column(Integer, unique=True, nullable=False)
...
class Layer3Domain(db.Model, TrackChanges):
    ...
    type: Mapped[String] = mapped_column(String(20), nullable=False)
    comment: Mapped[String] = mapped_column(String(255), nullable=True, index=False)
    # Vrf
    rd: Mapped[BigInteger] = mapped_column(BigInteger, nullable=True, unique=True)
    ...
    def set_rd(self, rd_str):
        ...
        if db.session.query(Layer3Domain).filter_by(rd=rd).count() > 0:
...
class Pool(db.Model, WithAttr):
    ...
    id: Mapped[BigInteger] = mapped_column(BigInteger, primary_key=True, nullable=False)
    name: Mapped[String] = mapped_column(String(128), unique=True, nullable=False)
    version: Mapped[Integer] = mapped_column(Integer, index=True, default=0, nullable=False)  # TODO allow null
    description: Mapped[String] = mapped_column(String(128))
    vlan_id: Mapped[BigInteger] = mapped_column(BigInteger, ForeignKey('vlan.id'))
    created: Mapped[TIMESTAMP] = mapped_column(TIMESTAMP, nullable=False,
                     default=datetime.datetime.utcnow,
                     server_default='1970-01-02 00:00:01')
    modified: Mapped[TIMESTAMP] = mapped_column(TIMESTAMP, nullable=False,
                      default=datetime.datetime.utcnow,
                      onupdate=datetime.datetime.utcnow,
                      server_default='1970-01-02 00:00:01')
    modified_by: Mapped[String] = mapped_column(String(128),
                         default=get_session_username,
                         onupdate=get_session_username)
    owner_id: Mapped[BigInteger] = mapped_column(BigInteger, ForeignKey('usergroup.id', ondelete='SET NULL'), nullable=True)
    layer3domain_id: Mapped[BigInteger] = mapped_column(BigInteger, ForeignKey('layer3domain.id'), nullable=False)
    ...
    def allocation_history(self, days_ago):
        return db.session.query(AllocationHistory)\
            .filter_by(pool=self)\
...
class PoolAttr(db.Model):
    ...
    id: Mapped[BigInteger] = mapped_column(BigInteger, primary_key=True, nullable=False)
    ippool_id: Mapped[BigInteger] = mapped_column(BigInteger, ForeignKey('ippool.id'), nullable=False)
    name_id: Mapped[BigInteger] = mapped_column('name', BigInteger, ForeignKey('ippoolattrname.id'), nullable=False)
    value: Mapped[String] = mapped_column(String(255), nullable=False)
...
class FavoritePool(db.Model):
    ...
    ippool_id: Mapped[BigInteger] = mapped_column(BigInteger, ForeignKey('ippool.id'), primary_key=True)
    user_id: Mapped[BigInteger] = mapped_column(BigInteger, ForeignKey('user.id'), primary_key=True)
...
class AllocationHistory(db.Model):
    ...
    ippool_id: Mapped[BigInteger] = mapped_column(BigInteger, ForeignKey('ippool.id'), nullable=False)
    total_ips: Mapped[Numeric] = mapped_column(Numeric(precision=40, scale=0), nullable=False)
    used_ips: Mapped[Numeric] = mapped_column(Numeric(precision=40, scale=0), nullable=False)

    pool = relationship(Pool, backref=backref('allocationhistory', cascade='all, delete-orphan'))

    @staticmethod
    def collect_data():
        for pool in db.session.query(Pool).all():
            db.session.add(
                AllocationHistory(
                    pool=pool,
...
class Ipblock(db.Model, WithAttr, TrackChanges):
    ...
    id: Mapped[BigInteger] = mapped_column(BigInteger, primary_key=True, nullable=False)
    version: Mapped[Integer] = mapped_column(Integer, index=True, nullable=False)
    address: Mapped[Numeric] = mapped_column(Numeric(precision=40, scale=0), nullable=False)
    prefix: Mapped[Integer] = mapped_column(Integer, nullable=False)
    priority: Mapped[Integer] = mapped_column(Integer)
    gateway: Mapped[Numeric] = mapped_column(Numeric(precision=40, scale=0))
    parent_id: Mapped[BigInteger] = mapped_column(BigInteger, ForeignKey('ipblock.id'))
    status_id: Mapped[BigInteger] = mapped_column(BigInteger, ForeignKey('ipblockstatus.id'))
    ippool_id: Mapped[BigInteger] = mapped_column(BigInteger, ForeignKey('ippool.id'))
    vlan_id: Mapped[BigInteger] = mapped_column(BigInteger, ForeignKey('vlan.id'))
    layer3domain_id: Mapped[BigInteger] = mapped_column(BigInteger, ForeignKey('layer3domain.id'), nullable=False)
    ...
    def _used(self, only_static) -> int:
        ret: int = 0
        q = db.session.execute(db.select(Ipblock.prefix, func.count()).filter(Ipblock.parent == self))
        if only_static:
    ...
    def delete(self):
        ...
        if not self.is_host:
            ...
            db.session.execute(db.update(Ipblock).filter_by(parent=self).values({'parent_id': self.parent_id}))
    ...
    def delete_subtree(self):
        ...
        for block in db.session.execute(db.select(Ipblock)\
                ...
                .order_by(Ipblock.prefix.desc())).scalars().all():
    ...
    def query_ip(ip, layer3domain, status=None):
        ...
        return db.session.execute(db.select(Ipblock).filter_by(address=ip.address,
                                       prefix=ip.prefix,
                                       version=ip.version,
                                       **filters)).scalar()
    ...
    def build_tree_mem(layer3domain, version):
        ...
        new_parents = []
        blocks = db.session.execute(db.select(Ipblock.id,
                                  Ipblock.address,
                                  Ipblock.prefix,
                                  Ipblock.parent_id)\
    ...
    def _tree_update(self):
        ...
        if not self.is_host:
            ...
            db.session.execute(db.update(Ipblock)\
    ...
    def _validate(self):
        ...
        if status == 'Subnet':
            ...
            if db.session.execute(db.select(Ipblock).join(IpblockStatus).filter(~or_(Ipblock.prefix == self.ip.bits, IpblockStatus.name == 'Delegation')).count()):
                raise Exception("Subnet %s cannot be created because it would have non-Delegation blocks as children" % self)
        elif status == 'Reserved':
            if db.session.execute(db.select(Ipblock).count()):
...
from sqlalchemy import Column, BigInteger, Integer, String, Numeric, TIMESTAMP, ForeignKey, UniqueConstraint
from sqlalchemy.ext.compiler import compiles
from sqlalchemy.orm import relationship, backref, validates, synonym, Mapped, mapped_column
from sqlalchemy.sql import bindparam, or_, between, func, expression, text
from sqlalchemy.types import DateTime
...
class Layer3Domain(db.Model, TrackChanges):
    ...
    comment: Mapped[str] = mapped_column(String(255), nullable=True, index=False)
    # Vrf
    rd: Mapped[BigInteger] = mapped_column(BigInteger, nullable=True, unique=True)
...
class Pool(db.Model, WithAttr):
    ...
    version: Mapped[int] = mapped_column(Integer, index=True, default=0, nullable=False)  # TODO allow null
    description: Mapped[str] = mapped_column(String(128))
    vlan_id: Mapped[BigInteger] = mapped_column(BigInteger, ForeignKey('vlan.id'))
    created: Mapped[datetime.datetime] = mapped_column(TIMESTAMP, nullable=False,
                     default=datetime.datetime.utcnow,
                     server_default='1970-01-02 00:00:01')
    modified: Mapped[datetime.datetime] = mapped_column(TIMESTAMP, nullable=False,
                      default=datetime.datetime.utcnow,
                      onupdate=datetime.datetime.utcnow,
                      server_default='1970-01-02 00:00:01')
    modified_by: Mapped[str] = mapped_column(String(128),
                         default=get_session_username,
                         onupdate=get_session_username)
    owner_id: Mapped[BigInteger] = mapped_column(BigInteger, ForeignKey('usergroup.id', ondelete='SET NULL'), nullable=True)
    layer3domain_id: Mapped[BigInteger] = mapped_column(BigInteger, ForeignKey('layer3domain.id'), nullable=False)

    vlan: Mapped[Vlan] = relationship(Vlan)
    owner: Mapped['Group'] = relationship('Group')
    layer3domain: Mapped[Layer3Domain] = relationship(Layer3Domain)
    ipblocks: Mapped[list["Ipblock"]] = relationship("Ipblock", backref="pool", order_by='Ipblock.priority')
    subnets = synonym('ipblocks')
...
class PoolAttr(db.Model):
    ...
    pool: Mapped[Pool] = relationship(Pool, backref=backref('attributes', cascade='all, delete-orphan'))
    name: Mapped[PoolAttrName] = relationship(PoolAttrName)
...
class FavoritePool(db.Model):
    ...
    user: Mapped['User'] = relationship('User', uselist=False, backref=backref('favorite_pools',
                                                               cascade='all, delete-orphan'))
    pool: Mapped['Pool'] = relationship('Pool', uselist=False, backref=backref('favorited_by',
                                                               cascade='all, delete-orphan',
                                                               collection_class=set))
...
class AllocationHistory(db.Model):
    id: Mapped[BigInteger] = mapped_column(BigInteger, primary_key=True, nullable=False)
    date: Mapped[datetime.datetime] = mapped_column(TIMESTAMP, nullable=False)
    ippool_id: Mapped[BigInteger] = mapped_column(BigInteger, ForeignKey('ippool.id'), nullable=False)
    total_ips: Mapped[Numeric] = mapped_column(Numeric(precision=40, scale=0), nullable=False)
    used_ips: Mapped[Numeric] = mapped_column(Numeric(precision=40, scale=0), nullable=False)

    pool: Mapped[Pool] = relationship(Pool, backref=backref('allocationhistory', cascade='all, delete-orphan'))
...
class Ipblock(db.Model, WithAttr, TrackChanges):
    ...
    # relationships
    parent: Mapped["Ipblock"] = relationship('Ipblock', remote_side=[id],
                          backref=backref('children', lazy='dynamic', order_by='Ipblock.address'))
    status: Mapped[IpblockStatus] = relationship(IpblockStatus)
    vlan: Mapped[Vlan] = relationship(Vlan)
    layer3domain: Mapped[Layer3Domain] = relationship(Layer3Domain)
    ...
    def _ancestors_noparent_query(ip, layer3domain, include_self=False):
        q = db.session.query(Ipblock).filter_by(version=ip.version)
        if layer3domain is not None:
...
class IpblockAttr(db.Model):
    __table_constraints__ = (UniqueConstraint('name', 'ipblock_id'), )

    id: Mapped[BigInteger] = mapped_column(BigInteger, primary_key=True, nullable=False)
    ipblock_id: Mapped[BigInteger] = mapped_column('ipblock', BigInteger, ForeignKey('ipblock.id'), nullable=False)
    name_id: Mapped[BigInteger] = mapped_column('name', BigInteger, ForeignKey('ipblockattrname.id'), nullable=False)
    value: Mapped[str] = mapped_column(String(255), nullable=False)

    ipblock: Mapped[Ipblock] = relationship(Ipblock, backref=backref('attributes', cascade='all, delete-orphan'))
    name: Mapped[IpblockAttrName] = relationship(IpblockAttrName)
