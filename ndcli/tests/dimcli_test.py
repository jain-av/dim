...
# Actual tests
def test_global_options():
    assert ndcli('invalid').returncode == 1
    assert ndcli('-D invalid').returncode == 32
    assert ndcli('-D -q -v').returncode == 32
...
class RightsTest(unittest.TestCase):
    ...
    def test_create_add_remove(self):
        assert user('create user-group usergroup').returncode != 0
        assert net('create user-group usergroup').ok
        assert user('modify user-group usergroup add user user').returncode != 0
        assert net('modify user-group usergroup add user user').ok
        assert user('delete user-group usergroup').returncode != 0
    ...
    def test_delete_group(self):
        ...
        assert user('list user-group networkgroup users').returncode != 0
        assert admin('delete user-group networkgroup').returncode != 0
        assert admin('create user-group networkgroup').ok
    ...
    def test_allocate(self):
        ...
        assert user('modify pool test_pool get ip').returncode != 0
    ...
    def test_revoke_network_admin(self):
        ...
        assert admin('modify user-group networkgroup revoke network_admin').ok
